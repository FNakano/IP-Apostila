...
int busca (float a[], int n) {
  int i;
  int imaior=-1; /* pode provocar segmentation fault na função de troca... */
  float vmaior;
  if (n>0) {
    // printf ("\nbusca %d:\n", n);
    imaior=0;
...
int main () {
  printf ("Último capítulo de uma série em 14 episódios!\n");
  imprime (a, n);

  for (int k=n-1;k>=0;k--) { /* decrescente pois posiciona o maior no final */
    int imaior;
    imaior=busca(a,k+1);   // olha uma eventual confusão com índices aqui!
    troca (a, imaior, k);
  }
  imprime (a, n);
  return 0;
...
Último capítulo de uma série em 14 episódios!
...
/* 11:45 - limpando comentários e depuração */
